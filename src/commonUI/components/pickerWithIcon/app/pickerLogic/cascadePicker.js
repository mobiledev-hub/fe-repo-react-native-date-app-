...
let modalHeight = Platform.OS === 'ios' ? 400 : (StatusBar.currentHeight + 400);
    ...
    render(){
        ...
        return (
            <View style={styles.container}>
                <Modal animationType={this.props.animationType} transparent={true} visible={this.state.visible}
                    ...
                    <View style={[styles.modalContainer]}>
                        ...
                        <Handle navStyle = {this.props.navStyle}
                                    ...
                                    if (this.props.loading.length > 0 && this.props.loading[index] && Platform.OS === 'android') {
                                        return (
                                            <View key={index} style={[{flex: 1, alignItems: 'center', borderWidth:0}, Platform.OS !== 'ios' && {justifyContent: 'center'}]}>
                                                {Platform.OS === 'ios' && <View style={{position: 'absolute', width: 500*em, height: 36*em, borderTopWidth: 1*em, borderBottomWidth: 1*em, borderColor: '#ddd', marginTop: 90*em}} />}
                                                <ActivityIndicator animating={true} size="small" style={Platform.OS === 'ios' && {marginTop: 96}}/>
                                            </View>
                                        );
                                    }
                                    return (
                                        <PickRoll
                                            itemStyle={{fontSize: 26*em, color: '#046390'}}
                                            id = {this.props.id} name = {this.props.name} parentId = {this.props.parentId}
                                            key = {index} wheelNumber = {index} style = {{flex:1}} pickerStyle = {{flex:1}}
                                            data = {that.passData[index]} passData = {that.passData}
                                            selectedIndex = {that.choseNumber[index]} selectedValue = {that.choseValue[index]}
                                            onValueChange={(newValue, newIndex) => {that._changeLayout(newValue, newIndex, index);}}>
                                            {Platform.OS === 'ios' && this.props.loading.length > 0 && this.props.loading[index] && (
                                                <PickerItem key={'loading'} value={'loading'} label={'loading'}/>
                                            )}
                                            {Platform.OS === 'ios' && !this.props.loading[index] && ((that.passData[index]).map((carMake) => (
                                                <PickerItem key={carMake} value={carMake[this.props.id]} label={carMake[this.props.name]}/>
                                            )))}
                                        </PickRoll>
                                    );
                                })}
