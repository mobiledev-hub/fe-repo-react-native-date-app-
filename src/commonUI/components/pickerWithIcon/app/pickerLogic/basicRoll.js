...
class Pickroll extends Component {
    ...
    */
    ...
    _renderItems(items){
        ...
        items.forEach((item, index) => {
            middleItems[index + 2] = <View style={rollStyles.textContainer} key={'mid' + (index + 2)}><Animated.Text
                ...
                style={[rollStyles.middleText, this.props.itemStyle,
                {
                    ...
                    // fontSize:
                    //   total.interpolate({
                    //     inputRange: [(index - 2) * itemHeight, index * itemHeight, (index + 2) * itemHeight],
        ...
        aroundItemArray.forEach((item, index) => {
            middleItems[item] = <View style={rollStyles.textContainer} key={'mid' + item}><Text
                className={'mid' + item}
                style={[rollStyles.middleTextDisable, this.props.itemStyle]} /></View>;
        });
    ...
    _calculateItemroll(type) {
        let diff;
        if (this.moveDy % itemHeight > (itemHeight / 2)) {
            diff = Math.ceil(this.moveDy / itemHeight);
            this.moveDy =  diff * itemHeight;
        } else {
            diff = Math.floor(this.moveDy / itemHeight);
            this.moveDy =  diff * itemHeight;
        }
    ...
    render(){
        ...
        return (
            <View style={{flex: 1}}>
                <View style={{position: 'absolute', width: 300*em, height: 80*em, borderTopWidth: 1*em, borderBottomWidth: 1*em, borderColor: '#ccc', marginTop: 75*em}} />
                <ScrollView
                    ref="_scrollView"
